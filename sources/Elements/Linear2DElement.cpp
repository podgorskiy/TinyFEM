...
std::vector<Eigen::Vector3f> Linear2DElement::GetFunctionValuesAtNodes(const Eigen::VectorXf& deforms)const
{
...
void Linear2DElement::CalcK(const StrideDataFixedArray& nodes, const tfem::MaterialPtr mat, std::vector<Eigen::Triplet<float> >& tripletVector)
{
	m_mat = mat;
	Eigen::Vector3f x;
	Eigen::Vector3f y;
...
tfem::Material* Linear2DElement::GetMaterial()
{
	return m_mat.get();
}
...
Linear2DElement::Linear2DElement()
{
