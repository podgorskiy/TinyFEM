...
// NOTE: These functions help to alleviate a GCC 4.7.2 bug on Debian Wheezy, when the compiler instantiates
//       pointer_traits on an incorrect pointer type. The compiler looses const qualifier from the element type
//       of the pointer (i.e. it instantiates pointer_traits<foo*> instead of pointer_traits<const foo*>).
template< typename To, typename From >
inline To static_cast_ptr(From const& p)
{
   return pointer_traits< To >::static_cast_from(p);
}

template< typename To, typename From >
inline To const_cast_ptr(From const& p)
{
   return pointer_traits< To >::const_cast_from(p);
}

template< typename To, typename From >
inline To dynamic_cast_ptr(From const& p)
{
   return pointer_traits< To >::dynamic_cast_from(p);
}

template< typename Ptr >
inline typename pointer_traits< Ptr >::pointer pointer_to(typename pointer_traits< Ptr >::reference obj)
{
   return pointer_traits< Ptr >::pointer_to(obj);
}
