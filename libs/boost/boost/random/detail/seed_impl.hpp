...
IntType get_one_int(Iter& first, Iter last)
{
    ...
    for(int j = 0; j < k; ++j) {
        if(first == last) {
            BOOST_THROW_EXCEPTION(::std::invalid_argument("Not enough elements in call to seed."));
        }
...
void fill_array_real(Iter& first, Iter last, RealType (&x)[n])
{
    ...
    for(j = 0; j < n; ++j) {
        ...
        for(int k = 0; k < w/32; ++k, ++first) {
            if(first == last) BOOST_THROW_EXCEPTION(std::invalid_argument("Not enough elements in call to seed."));
            val += *first * mult;
            mult *= two32;
        }
        if(mask != 0) {
