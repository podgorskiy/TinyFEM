...
extern "C" __declspec(dllimport) unsigned long __stdcall FormatMessageW
   ...
   unsigned long dwLanguageId,   wchar_t *lpBuffer,         unsigned long nSize,
    ...
    char *lpBuffer, unsigned long nSize, std::va_list *Arguments)
{
	...
	unsigned long num_char = FormatMessageW(dwFlags, lpSource, dwMessageId, dwLanguageId, &wstr[0], nSize, Arguments);
	WideCharToMultiByte(CP_ACP, 0, wstr.c_str(), -1, lpBuffer, num_char, NULL, NULL);
