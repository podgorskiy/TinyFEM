...
struct OcornutImguiContext
{
	...
	{
		m_viewId = _viewId + _inputChar * 0;
		ImGuiIO& io = ImGui::GetIO();
		const uint8_t* inputChar = inputGetChar();
		if (inputChar != NULL)
		{
			if (*inputChar < 0x7f)
			{
				io.AddInputCharacter(static_cast<ImWchar>(*inputChar)); // ASCII or GTFO! :(
			}
		}
		...
		{
			bool state = inputGetKeyState(entry::Key::Enum(ii));
			io.KeysDown[ii] = state;
		}
