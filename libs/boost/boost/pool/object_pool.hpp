...
template <typename T, typename UserAllocator, bool Ordered>
...
public:
	...
	struct malloc_guard
	{
		malloc_guard(element_type* ptr_, object_pool<element_type, user_allocator, Ordered>* pool_)
			: ptr(ptr_)
			, free_ptr(true)
			, pool(pool_)
		{
		...
		element_type* ptr;
		bool free_ptr;
		object_pool<element_type, user_allocator, Ordered>* pool;
	};
  ...
  public:
    ...
    { //! De-Allocates memory that holds a chunk of type ElementType.
      ...
      //!
      //! Note that the destructor for p is not called. O(N), O(1) in not ordered.
	  if(Ordered)
    ...
    */
	...
	{
		pool<UserAllocator>::swap(other);
		std::swap(allocation_count, other.allocation_count);
	}
...
#   include <boost/pool/detail/pool_construct_simple.ipp>
    ...
    //! Sorts all allocations; requires to release memory.
    std::pair<details::PODptr<size_type>, void*> sort_allocation()
	{
	  details::PODptr<size_type> iter = this->list;
	  void * freed_iter = this->first;

	  // need to sort the allocation if unordered.
	  if(!Ordered)
	  {
		// sort list
		{
		  details::PODptr<size_type> sorted;

		  while(iter.valid())
		  {
			details::PODptr<size_type> head = iter;
			details::PODptr<size_type> trail = sorted;
			...
			for(;;)
			{
			  if(!trail.valid() || head.begin() < trail.begin())
			  {
				head.next(trail);
		...
		// sort free list: space complexity: O(1) - time complexity O(n^2)
		{
		  void * x = freed_iter;
		  void * l = 0;
		  while(x != 0)
		...
		// sort free list: space complexity: O(n) - time complexity O(n.log(n))
		/*
		{
		  // Count List
		  size_t size = 0;
		  ...
		  // Populate array
		  void ** v = reinterpret_cast<void**>((UserAllocator::malloc)(sizeof(void*) * (size + 1)));
		  size_t k = 0;
		  i = freed_iter;
		  while(i != NULL)
		  {
		  ...
		  {
			*static_cast<void**>(v[i]) = v[i+1];
		  }

		  (UserAllocator::free)(reinterpret_cast<char*>(v));
		}
		*/
	  }

	  return std::make_pair(iter, freed_iter);
	}

	//! Ordering-safe release_memory.
	bool release_memory()
	{
	  sort_allocation();
...
template <typename T, typename UserAllocator, bool Ordered>
object_pool<T, UserAllocator, Ordered>::~object_pool()
{
  ...
  std::pair<details::PODptr<size_type>, void*> result = sort_allocation();
  details::PODptr<size_type> iter = result.first;
  void * freed_iter = result.second;
  details::PODptr<size_type> next = iter;
  ...
  do
  {
    // increment next
    next = next.next();
    BOOST_ASSERT(!next.valid() || iter.begin() < next.begin());
    ...
    for (char * i = iter.begin(); i != iter.end(); i += partition_size)
    {
      ...
      if (i == freed_iter)
      {
        ...
        BOOST_ASSERT(freed_iter == NULL || i < freed_iter);
  ...
  BOOST_ASSERT(allocation_count == 0);
