...
inline BOOST_IOSTREAMS_FAILURE system_failure(const char* msg)
{
	...
	if(err != NO_ERROR)
	{
		...
		for (;;)
		{
			...
			{
				return BOOST_IOSTREAMS_FAILURE("Unknown error");
			}
			else
			{
				wstr.resize( wstr.size() + wstr.size()/2 );
			}
		}

		std::string str;
		str.resize(wstr.size());
		WideCharToMultiByte( CP_ACP, 0, wstr.c_str(), -1, &str[0], wstr.size(), NULL, NULL );

		result.reserve(std::strlen(msg) + 2 + str.length());
		result.append(msg);
		result.append(": ");
		result.append(str);
	}
